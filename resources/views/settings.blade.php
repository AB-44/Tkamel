...
<body>
@include('layouts.sidebar-admin', ['activeNav' => 'settings'])
...
{{-- Toast notification (match other pages) --}}
  ...
  async function saveChanges(section) {
    ...
    try {
      ...
      if (section === 'security') {
        ...
        const data = await res.json();
        if (!res.ok || data.success === false) {
          const msg = data?.errors?.current_password?.[0] || data?.errors?.new_password?.[0] || data?.errors?.confirm_password?.[0] || data?.message || 'تعذر تحديث كلمة المرور';
          showToast(msg, 'error');
          return;
        }
